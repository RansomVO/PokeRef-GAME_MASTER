...
namespace VanOrman.PokemonGO.GAME_MASTER.Parser.Templates
{
    public class PokemonTranslator : TemplateTranslator
    {
        ...
        public override string Key { get { return string.Equals(base.Key, "NIDORAN") ? PokemonSettings.pokemonId : base.Key; } }
        ...
        public string Name { get { return FixPokemonName(PokemonSettings.pokemonId) + (Form == null ? string.Empty : (" (" + Form + ")")); } }

        public string Form { get { return Key.Length == PokemonSettings.pokemonId.Length || Key.EndsWith(MARKER_GENDER) ? null : TemplateTranslator.FixName(Key.Substring(PokemonSettings.pokemonId.Length + 1)); } }

        public string Type1 { get { return TemplateTranslator.FixName(PokemonSettings.type.Substring(MARKER_TYPE.Length)); } }

        public string Type2 { get { return string.IsNullOrEmpty(PokemonSettings.type2) ? string.Empty : TemplateTranslator.FixName(PokemonSettings.type2.Substring(MARKER_TYPE.Length)); } }

        public string CandyType { get { return TemplateTranslator.FixName(PokemonSettings.familyId.Substring(MARKER_CANDY.Length)); } }

        public string Rarity { get { return string.IsNullOrEmpty(PokemonSettings.rarity) ? string.Empty : TemplateTranslator.FixName(PokemonSettings.rarity.Substring(MARKER_RARITY.Length)); } }
            ...
            if (PokemonSettings.evolutionBranch != null)
            {
                foreach (var evolution in PokemonSettings.evolutionBranch)
                {
                    if (pokemon.ContainsKey(evolution.evolution))
                    {
                        ...
                        pokemon[evolution.evolution].EvolveSpecialItem = evolution.evolutionItemRequirement == null ? null : TemplateTranslator.FixName(evolution.evolutionItemRequirement.Substring(MARKER_EVOLVE_SPECIAL_ITEM.Length));
                    }
                }
            }
