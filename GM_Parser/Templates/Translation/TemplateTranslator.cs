...
namespace VanOrman.PokemonGO.GAME_MASTER.Parser.Templates
{
    public class TemplateTranslator
    {
        ...
        public string FixKey(string rawKey)
        {
            foreach(var key in MARKERS_TEMPLATE_KEY_)
                if (Regex.Match(rawKey, key.Key).Success)
                    return rawKey.Substring(key.Value == -1 ? key.Key.Length : key.Value);
