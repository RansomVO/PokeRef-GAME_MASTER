...
namespace VanOrman.PokemonGO.GAME_MASTER.DataGenerator.Templates.DataFiles
{
	...
	public class Encounter
	{
					...
					if (research.Encounter != null)
						foreach (var encounter in research.Encounter)
							if (!written.Contains(encounter.id))
							{
								...
								upToDate = WriteEncounter(encounter, fieldResearch.last_updated, gameMasterStatsCalculator.GameMasterStats.last_updated.Date) && upToDate;
							}
		...
		private static bool WriteEncounter(_Encounter encounter, DateTime encountersLastUpdated, DateTime gameMasterLastUpdated)
		{
			bool upToDate = true;
			string encounterFileName = GetFileNameBase(encounter);
			...
			if (!File.Exists(filePath) ||
				lastUpdated < encountersLastUpdated ||
				lastUpdated < gameMasterLastUpdated)
			{
				Utils.WriteXML(new Encounter(encounter, Utils.GetEncounterPossibilities(encounter.PokemonTranslator, 15)), filePath);
				upToDate = false;
			}
